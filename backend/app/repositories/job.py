...
class JobRepositoryImpl:
    ...
    def get_jobs_ready_for_retry(self, db: Session, now: datetime, max_retries: int) -> list[Job]:
    ...
    def delete_by_video_id(self, db: Session, video_id: uuid.UUID) -> int:
        jobs = self.get_by_video_id(db, video_id)
        count = len(jobs)
        for job in jobs:
            db.delete(job)
        db.commit()
...
job_repository: JobRepository = JobRepositoryImpl()