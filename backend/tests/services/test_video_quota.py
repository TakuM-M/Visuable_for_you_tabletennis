...
def test_quota_not_exceeded_passes() -> None:
    ...
    db = MagicMock()
    user_id = uuid.uuid4()
    with patch("app.services.video_service.video_repo.count_by_user_id", return_value=5), \
...
def test_quota_just_under_limit_passes() -> None:
    ...
    db = MagicMock()
    user_id = uuid.uuid4()
    with patch("app.services.video_service.video_repo.count_by_user_id", return_value=9), \
         patch("app.services.video_service.settings.user_video_quota", 10):
        video_service._ensure_under_quota(db, user_id)
...
def test_quota_at_limit_raises() -> None:
    ...
    db = MagicMock()
    user_id = uuid.uuid4()
    with patch("app.services.video_service.video_repo.count_by_user_id", return_value=10), \
         patch("app.services.video_service.settings.user_video_quota", 10):
...
def test_quota_exceeded_raises() -> None:
    ...
    db = MagicMock()
    user_id = uuid.uuid4()
    with patch("app.services.video_service.video_repo.count_by_user_id", return_value=15), \
         patch("app.services.video_service.settings.user_video_quota", 10):
