...
def test_reap_timeouts_handles_each_timed_out_job() -> None:
    ...
    with patch("app.services.job_reaper.SessionLocal") as sl, \
...
def test_dispatch_retries_prepares_and_rekicks() -> None:
    ...
    with patch("app.services.job_reaper.SessionLocal") as sl, \
         patch("app.services.job_reaper.job_repo.get_jobs_ready_for_retry", return_value=[job]), \
...
def test_dispatch_retries_noop_when_no_jobs() -> None:
    with patch("app.services.job_reaper.SessionLocal") as sl, \
         patch("app.services.job_reaper.job_repo.get_jobs_ready_for_retry", return_value=[]), \
...
def test_log_storage_metrics_logs_on_success() -> None:
    ...
    with patch("app.services.job_reaper.SessionLocal") as sl, \
         patch("app.services.job_reaper.metrics_service.collect_storage_metrics",
               return_value=metrics) as collect:
        sl.return_value.__enter__.return_value = MagicMock()
...
def test_clean_tmp_dir_removes_old_files_and_keeps_new(tmp_path) -> None:
    ...
    with patch("app.services.job_reaper.LOCAL_TMP_DIR", tmp_path), \
