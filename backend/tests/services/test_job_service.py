...
def test_compute_next_retry_at_uses_backoff_for_index() -> None:
    with patch("app.services.job_service.settings.job_retry_backoff_seconds", [60, 600]):
...
def test_send_failure_notification_skips_when_video_missing() -> None:
    db = MagicMock()
    with patch("app.services.job_service.video_repo.get_by_id", return_value=None), \
...
def test_send_failure_notification_skips_when_user_missing() -> None:
    db = MagicMock()
    video = _make_video()
    with patch("app.services.job_service.video_repo.get_by_id", return_value=video), \
...
def test_send_failure_notification_records_sent_on_success() -> None:
    ...
    with patch("app.services.job_service.video_repo.get_by_id", return_value=video), \
         ...
         patch("app.services.job_service.notification_log_repo.create", return_value=log), \
         patch("app.services.job_service.send_clip_failure_email", return_value=True) as send_mail, \
         patch("app.services.job_service.notification_log_repo.update_status") as update_status:
        job_service._send_failure_notification(db, uuid.uuid4(), video.id, "err")
...
def test_retry_job_404_when_job_missing() -> None:
    ...
    with patch("app.services.job_service.job_repo.get_by_id", return_value=None):
        with pytest.raises(HTTPException) as exc:
            job_service.retry_job(db=db, job_id=uuid.uuid4(), current_user=cu, background_tasks=bt)
...
def test_retry_job_404_when_video_missing() -> None:
    ...
    with patch("app.services.job_service.job_repo.get_by_id", return_value=job), \
        ...
        with pytest.raises(HTTPException) as exc:
            job_service.retry_job(db=db, job_id=job.id, current_user=cu, background_tasks=bt)
    assert exc.value.status_code == 404
...
def test_process_complete_job_delegates_to_handle_on_error() -> None:
    job_id = uuid.uuid4()
    with patch("app.services.job_service.SessionLocal") as sl, \
         patch("app.services.job_service.complete_job", side_effect=RuntimeError("boom")), \
