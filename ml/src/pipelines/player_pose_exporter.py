...
class PlayerPoseExporter:
    ...
    def __init__(self, config: PlayerPoseExporterConfig):
        ...
        if config.save_intermediate_files:
            self.visualizer = PlayerClassifierVisualizer(
                self.table_detector, self.pose_tracker, self.player_classifier
            )
    ...
    def _initialize_video_processing(
        self, input_video: str, output_video: str, csv_output: str, target_fps: float
        ...
        if self.visualizer is not None:
            ...
            video_writer = cv2.VideoWriter(
                output_video, fourcc, target_fps, (width, height)
            )
            if not video_writer.isOpened():
                raise VideoProcessingError(
                    f"VideoWriterの初期化に失敗しました: {output_video}"
                )
    ...
    def _print_video_info(self, video_info: Dict[str, Any]):
        ...
        print(
            f"  出力動画FPS: {video_info['target_fps']:.2f} (処理したフレームのみ出力)\n"
        )

        print(f"プレイヤー分類設定:")
        print(f"  最大プレイヤー数: {self.config.player_classification.max_players}")
        print(
            f"  最小スコア閾値: {self.config.player_classification.min_player_score:.2f}\n"
        )

    def _detect_table_with_validation(self, cap: cv2.VideoCapture) -> TableInfo:
        """卓球台検出と検証"""
    ...
    def _detect_table(
        ...
        for attempt in range(max_attempts):
            ...
            if not ret:
                print(
                    f"警告: フレーム読み込み失敗（試行 {attempt + 1}/{max_attempts}, フレーム位置: {frame_pos}）"
                )
                continue
            ...
            table_info = self.table_detector.detect_table_frame(
                frame=frame, frame_idx=actual_frame_idx, force_detect=True
            )
            ...
            if table_info is not None and table_info.confidence >= min_confidence:
                ...
                print(
                    f"  信頼度不足: {table_info.confidence:.3f} < {min_confidence:.2f} "
    ...
    def _print_table_detection_success(
        ...
        print(
            f"  フレーム位置: {frame_idx}/{total_frames} ({frame_idx / total_frames * 100:.1f}%)"
        )
    ...
    def _print_processing_info(self, video_info: Dict[str, Any]):
        ...
        print("処理開始...\n")
        print(
            f"  フレームステップ: {video_info['frame_step']} "
            f"({video_info['frame_step']}フレームごとに1回処理)"
        )
        print(
            f"  予測処理フレーム数: 約{video_info['total_frames'] // video_info['frame_step']}フレーム"
        )
    ...
    def _create_display_frame(
        ...
        display_frame = self.visualizer.draw_candidate_info(display_frame, player_ids)
    ...
    def _print_results(
        self, frame_count: int, processed_count: int, player_ids: Set[int]
    ):
        """処理結果を表示"""
        print(f"\n✓ 処理完了:")
    ...
    def _cleanup_resources(
        self, cap: cv2.VideoCapture, video_writer: Optional[cv2.VideoWriter]
    ):
        """リソースのクリーンアップ"""
        cap.release()
