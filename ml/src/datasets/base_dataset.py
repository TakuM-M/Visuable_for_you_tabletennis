...
class BasePoseSequenceDataset(Dataset, ABC):
    ...
    def _compute_motion_features(self) -> None:
        ...
        # 結合: (num_frames, 102)
        self.features = np.concatenate([coords, velocity, acceleration], axis=1).astype(
            np.float32
        )
    ...
    def _create_sequence_indices(self) -> List[Tuple[int, int]]:
        ...
        if self.features is None:
            raise ValueError(
                "featuresが設定されていません。先に_load_dataを呼び出してください。"
            )
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, Dict]:
        ...
        if idx < 0 or idx >= len(self.sequence_indices):
            raise IndexError(
                f"インデックス {idx} が範囲外です [0, {len(self.sequence_indices)})"
            )
