"""
プレイヤーを継続して追跡するためのテストコード
選手が画面領域外に出た場合でも継続して検出しづつけることを目標とする
...
処理の流れ:
1. 初回のみ画面中央にいる人物を検出（他の人間検出を避けるため）
2. 検出した人物をトラッキングし続ける（検出範囲は画面全体）
3. トラッキング中の姿勢データを抽出・保存

使い方:
---------
# Webカメラから検出（リアルタイム）
python tests/detect_player.py

# 動画ファイルから検出
python tests/detect_player.py -i path/to/video.mp4

# 結果を動画として保存
python tests/detect_player.py -i video.mp4 -o output.mp4

# 姿勢データをCSVに保存
python tests/detect_player.py -i video.mp4 --csv pose_data.csv

# 動画とCSVの両方を保存
python tests/detect_player.py -i video.mp4 -o output.mp4 --csv pose_data.csv

# 中央検出領域を変更（画面の40%）
python tests/detect_player.py -i video.mp4 --center-ratio 0.4

# 検出信頼度の閾値を調整
python tests/detect_player.py -i video.mp4 --conf 0.6

機能:
-----
- CenterPlayerDetector: 画面中央のプレイヤーを検出し、画面全体でトラッキング
- PoseDataExporter: 姿勢キーポイント（17点）をCSVに出力
- リアルタイムで骨格とバウンディングボックスを表示
...
- timestamp: タイムスタンプ（秒）
- track_id: トラッキングID
- bbox_x1, bbox_y1, bbox_x2, bbox_y2: バウンディングボックス
- confidence: 検出信頼度
- [キーポイント名]_x, _y, _conf: 各キーポイントの座標と信頼度
...
def main():
    """メイン処理 - Webカメラまたは動画からプレイヤーを検出・トラッキング"""
    import argparse
